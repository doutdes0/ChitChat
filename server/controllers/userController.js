...
const refreshToken = async (req, res, next) => {
  try {
    ...
    if (!cookies.jwt)
      return res.status(401).json({ msg: "No refresh token cookie" });
    const refreshToken = cookies.jwt;
    const user = await User.findOne({ refreshToken });
    if (!user)
      res.status(401).json({ msg: "User with this refresh token not found" });
    jwt.verify(
      refreshToken,
      process.env.REFRESH_TOKEN_SECRET,
      (err, decoded) => {
        if (err || user.username !== decoded.username) {
          return res
            .status(401)
            .json({ msg: "Invalid token(belongs to another user)" });
        }
        const accessToken = jwt.sign(
...
const getAllUsers = async (req, res, next) => {
  try {
    ...
    const list = await User.find({ username: { $ne: username } }).select(
      "avatar username _id"
    );
